...
<meta name="viewport" content="width=device-width,initial-scale=1.0; maximum-scale=1.0;"/><link rel="stylesheet" href="/m.css" type="text/css"/><title>Lyrics Editing :: WapSmsBD.Com</title></head>';
echo '<body>';
print '<div class="logo" align="left"><div><a href="/"><font size="5" color="white">WapSmsBD.Com</font></a></div></div>';
print '<div class="h1">Edit Lyrics</div>';
echo '<div class="box">';
...
$d_open = fopen("extra/lyricsTxt/lid_$sid.txt","w+");
$lwrite=fwrite($d_open, $lyrics);
$lclose=fclose($d_open);
if($lwrite && $lclose){print '<font color="green">Lyrics Edited Successfully !</font>';}
...
print '<form action="" method="post">Lyrics Edit : <br/><input type="hidden" name="gid" value="'.$sid.'"><textarea name="lyrics" cols=40 rows=6>';

$dfile = file("extra/lyricsTxt/lid_$sid.txt");
/// Get Data
...
print '<form action="imupload.php" method="post"><input type="hidden" name="id" value="'.$sid.'">Art Url :<br/><input type="text" name="url" value=""><br/><input type="submit" name="arturl" value="submit"></form>';

print '<form action="imupload.php" method="post" enctype="multipart/form-data"><input type="hidden" name="id" value="'.$sid.'">
    Select Art:<br/>
    <input type="file" name="art" id="art"><br/>
    <input type="submit" value="Upload" name="asubmit">
</form>';

print '<form action="imupload.php" method="post"><input type="hidden" name="id" value="'.$sid.'">Art Url :<br/><input type="text" name="url" value=""><br/><input type="submit" name="thurl" value="submit"></form>';
...
print '<form action="" method="post"><input type="hidden" name="gid" value="'.$sid.'">Album :<br/><input type="text" name="album" value="'.urldecode($sqlalbum[0]).'"><br/><input type="submit" name="SubmitAlbum" value="Edit"></form>';

print '<form action="" method="post"><input type="hidden" name="gid" value="'.$sid.'">Singer : <br/><input type="text" name="singer" value="'.urldecode($sqlsinger[0]).'"><br/><input type="submit" name="SubmitSinger" value="Edit"></form>';
