...
class filesActions extends sfActions
{
		...
		if($this->getRequestParameter('url_path') && $this->getRequestParameter('urlType') == 'CopyToServer')
		{
			...
			*/
			if($type=='w')
			{
				...
				myClass::getImageRatio($filePathToSave,'thumb/'.$thumbServer.'/',70, 70, $width1, $height1,false, 'small_'.$files->getId().'.jpg',false,80);
				myClass::getImageRatio($filePathToSave,'thumb/'.$thumbServer.'/',110, 110, $width1, $height1,false, 'mid_'.$files->getId().'.jpg',false,80,'logo_v.png');
				myClass::getImageRatio($filePathToSave,'thumb/'.$thumbServer.'/',150, 150, $width1, $height1,false, 'thumb_'.$files->getId().'.jpg',false,80,'logo_v.png');
		...
		if($this->getRequest()->getFileSize('file_name') > 0)
		{
			...
			*/
			...
			if($type=='w')
			{
				...
				myClass::getImageRatio($fPath,'thumb/'.$thumbServer.'/',70, 70, $width1, $height1,false, 'small_'.$files->getId().'.jpg',false,80);
				myClass::getImageRatio($fPath,'thumb/'.$thumbServer.'/',110, 110, $width1, $height1,false, 'mid_'.$files->getId().'.jpg',false,80);
				myClass::getImageRatio($fPath,'thumb/'.$thumbServer.'/',150, 150, $width1, $height1, false, 'thumb_'.$files->getId().'.jpg',false,80, 'logo_v.png');
		...
		if($this->getRequestParameter('dirCatch'))
		{
			if($files->getExtension()!='JPG' && $files->getExtension()!='JPEG' && $files->getExtension()!='PNG'):
				...
				if($fileHasThumb!=false)
				{
					...
					myClass::getImageRatio($fileHasThumb,'thumb/'.$thumbServer.'/',70, 70, $width1, $height1,false, 'small_'.$files->getId().'.jpg',true,80,'logo.png');
					myClass::getImageRatio($fileHasThumb,'thumb/'.$thumbServer.'/',110, 110, $width1, $height1,false, 'mid_'.$files->getId().'.jpg',true,80,'logo_v.png');
					myClass::getImageRatio($fileHasThumb,'thumb/'.$thumbServer.'/',150, 150, $width1, $height1,false, 'thumb_'.$files->getId().'.jpg',true,80,'logo_v.png');
		...
		*/
		if($this->getRequest()->getFileSize('thumb_name') > 0 && ($files->getExtension()!='JPG' && $files->getExtension()!='JPEG' && $files->getExtension()!='PNG'))
		{
			...
			if(in_array(strtolower(end($thumbExt)),$wallpaper))
			{
				...
				myClass::getImageRatio($this->getRequest()->getFilePath('thumb_name'),'thumb/'.$thumbServer.'/',70, 70, $width1, $height1,false, 'small_'.$files->getId().'.jpg',true,80,'logo.png');
				myClass::getImageRatio($this->getRequest()->getFilePath('thumb_name'),'thumb/'.$thumbServer.'/',110, 110, $width1, $height1,false, 'mid_'.$files->getId().'.jpg',true,80,'logo_v.png');
				myClass::getImageRatio($this->getRequest()->getFilePath('thumb_name'),'thumb/'.$thumbServer.'/',150, 150, $width1, $height1,false, 'thumb_'.$files->getId().'.jpg',true,80,'logo_v.png');
			}
		...
		if($this->getRequestParameter('thumb_url_path')){
			...
			curl_exec($ch) or die('error3');
			//$info = curl_getinfo($ch);
			fclose($outfile);

			list($width1, $height1, $type1, $attr1) = getimagesize($thumbSave);

			myClass::getImageRatio($thumbSave,'thumb/'.$thumbServer.'/',70, 70, $width1, $height1,false, 'small_'.$files->getId().'.jpg',true,80,'logo.png');
			myClass::getImageRatio($thumbSave,'thumb/'.$thumbServer.'/',110, 110, $width1, $height1,false, 'mid_'.$files->getId().'.jpg',true,80,'logo_v.png');
			myClass::getImageRatio($thumbSave,'thumb/'.$thumbServer.'/',150, 150, $width1, $height1,false, 'thumb_'.$files->getId().'.jpg',true,80,'logo_v.png');
		}
		...
		if($files->getExtension()=='MP3' && $this->getRequestParameter('mp3tags')=='true' && ($this->getRequestParameter('url_path') || $this->getRequest()->getFileSize('file_name') > 0) ){
			header('Location: /skyadnin/SkyWriteMp3.php?Filename='.$dataServer.'/'.$files->getId().'/'.$files->getFileName().'&cid='.$files->getCategoryId().'&catName='.base64_encode(CategoryPeer::getCategoryName($files->getCategoryId())).'&lastFile='.base64_encode($files->getFileName()) );
			exit;
		}
		else
  ...
  public function executeActivation()
  {
    ...
    $this->files->save();
  	if($this->getRequestParameter('status') == 'B' || $this->getRequestParameter('status') == 'F')
  		echo '<a onclick="new Ajax.Updater(\'active_'.$this->getRequestParameter('id').'\', \'/skyadnin/files/activation/id/'.$this->getRequestParameter('id').'/status/A\', {asynchronous:true, evalScripts:false});; return false;" href="#">Active</a>';
  	else
  		echo '<a onclick="new Ajax.Updater(\'active_'.$this->getRequestParameter('id').'\', \'/skyadnin/files/activation/id/'.$this->getRequestParameter('id').'/status/B\', {asynchronous:true, evalScripts:false});; return false;" href="#"><span class="red">Block</span></a>';
		return sfView::NONE;
  }
