...
######## Search ###########
 print '<div class="search">Search Files:<form method="get" action="/search.php"><input type="text" name="search" class="input"/> <input type="submit" value="Search" class="button"/></form></div>';
...
print $row.$ico.'<a href="/wap/'.$dirt.'.html"><strong>'.transdir($dir_exp[count($dir_exp)-1]).'</strong></a> ('.$tot.')</div>';
...
print $row.''.$thumb.'</td><td class="left"><a class="fileName" href="/wfile/'.$glob_file[$i].'.html">'.$basename.' </a><br/>('.$filesize.') </td></tr>';
...
if ($prev>0) $pgprev='<a class="page" href="/wx/'.$dir.'-'.$prev.'.html">'.$prev.'</a>';

if ($next<$countstr) $pgnext='<a class="page" href="/wx/'.$dir.'-'.$next.'.html">'.$next.'</a>';

if($page>2) $first='<a class="page" href="/wx/'.$dir.'-1.html">1</a>';

if($page<$countstr) $pglast='<a class="page" href="/wx/'.$dir.'-'.$countstr.'.html">'.$countstr.'</a>';
