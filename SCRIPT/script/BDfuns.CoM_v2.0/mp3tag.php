...
echo '<html><head>';
echo '<title>getID3() - Mp3 Tag Edit</title>';
		...
		foreach ($DirectoryContents as $dirname => $val) {
			...
			if (isset($DirectoryContents[$dirname]['known']) && is_array($DirectoryContents[$dirname]['known'])) {
				...
				foreach ($DirectoryContents[$dirname]['known'] as $filename => $fileinfo) {
					...
					echo '</td>';
					echo '<td align="left">&nbsp;<a href="/">Author</a></td>';
					echo '</tr>';
				}
			}
			...
			if (isset($DirectoryContents[$dirname]['other']) && is_array($DirectoryContents[$dirname]['other'])) {
				...
				foreach ($DirectoryContents[$dirname]['other'] as $filename => $fileinfo) {
					...
					echo '<td align="left">&nbsp;<a href="http://fb.com/shaon1993">Author</a></td>';
					echo '</tr>';
				}
			}
...
function PoweredBygetID3($string='<br><HR NOSHADE><DIV STYLE="font-size: 8pt; font-face: sans-serif;">Powered by <a href="./index.php/" TARGET="_blank"><b>Email : v.djshaon@gmail.com</b>http://bdfuns.com<br></a></DIV><div class="logo"><center></center></div><center></center>') {
