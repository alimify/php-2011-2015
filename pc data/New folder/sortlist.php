...
<meta name="Identifier-URL" content="http://www.anymaza.com"/><meta http-equiv="Cache-control" content="no-cache"><link rel="shortcut icon" href="http://www.bollyclassic.com/uploads/favicon.ico"/><link rel="stylesheet" href="/css/style.css" type="text/css" /></head>
<body>';
...
echo"</center>";
if($sort)
{print '<div class="tags"><center>Sort by: <a href="http://AnyMaza.Com/srt/'.$fldid.'/'.$seotitle.'/0.html">A To Z</a>/New To Old</center></div>';}
else
...
print '<div class="folder"><a href="http://AnyMaza.Com/'.$folderid[0].'/'.transdir($dir_exp[count($dir_exp)-1]).'.html">'.transdir($dir_exp[count($dir_exp)-1]).' ['.$mcount.']</a></div>';
}
}
}
//// FileList
if(file_exists('file/data/zipfile/'.$fldid.'.zip')){ 
$zipfile = "file/data/zipfile/'.$fldid.'.zip";
$zipsize=filesize($zipfile);
if($zipsize>1048576)
$zipsize=round($zipsize/1048576, 2).' Mb';
elseif($zipsize>1024)
$zipsize=round($zipsize/1024, 2).' Kb'; print '<a href="/zip/'.$fldid[0].'/'.$seotitle.' Full Album.html"><div class="file"><table><tr class="fl odd"><td width="16%"><img src="/ext/zip.gif"  width="70px" height="50px" alt="'.$seotitle.' Full Album Zip File thumb" /></td><td><b>Full Zipped Album</b><br/><font color="black"><small>File Size: '.$zipsize.'</small></font></td></tr></table></div></a>'; }
...
///URL SETUP
if(r($glob_file[$i])=='mp3') $urlset= '<div class="file"><a href="/view/'.$fileid[0].'/'.$title.' By '.$artist.'.html.html">'.$title.'<br/><font color="black"><small>'.$reartist.'</small></font></a></div>';
...
//Pagination:
if($countstr>1)
{echo '<div class="pgn"><center>';print nav_page($countstr,$fldid,$seotitle,$sort,$page,'index');}
echo'</center></div>';
include("ads/buzzcity.php");
